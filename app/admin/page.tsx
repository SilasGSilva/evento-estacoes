...
import { Banknote, Check, Copy, Download, Search, ShieldCheck, Trash2 } from 'lucide-react';
...
function getStatusPagamentoClassName(statusPagamento: string) {
  return isPagamentoPago(statusPagamento)
...
export default function AdminPage() {
  ...
  const [sortByNameAZ, setSortByNameAZ] = useState(false);
  ...
  const copiarTelefone = async (whatsapp: string) => {
    try {
      const telefone = getTelefoneWhatsApp(whatsapp);
      if (!telefone) {
        toast.error('WhatsApp inválido para cópia.');
        return;
      }
  ...
  const inscritosFiltrados = useMemo(() => {
    ...
    return filtrados;
  }, [busca, filtroStatus, inscritos, sortByNameAZ]);

  const totalInscritos = inscritos.length;
  const totalPagos = inscritos.filter(item => isPagamentoPago(item.status_pagamento)).length;
  const totalPendentes = inscritos.filter(item =>
    isPagamentoPendente(item.status_pagamento)
  ).length;
        ...
        <section className="rounded-2xl border border-accent/70 bg-surface p-4 shadow-md">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setSortByNameAZ(prev => !prev)}
                className={`rounded-full px-3 py-2 text-sm font-semibold transition ${
                  sortByNameAZ
                    ? 'bg-secondary text-surface shadow-lg shadow-secondary/30'
                    : 'border border-gray-200 bg-gray-50 text-primary hover:border-secondary/60 hover:bg-gray-100'
                }`}
                title="Ordenar por nome A–Z"
              >
                {sortByNameAZ ? 'Nome A–Z' : 'Ordenar Nome A–Z'}
              </button>
          ...
          <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              {FILTROS_STATUS.map(filtro => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`rounded-full px-2 py-0.5 text-xs font-bold ${filtro.badgeClassName}`}
                    >
            ...
            <span className="text-sm font-medium text-primary/70 md:text-right">
              Exibindo <strong>{inscritosFiltrados.length}</strong> de{' '}
            ...
            <section className="space-y-4 md:hidden">
              {inscritosFiltrados.length === 0 ? (
                ...
                inscritosFiltrados.map(inscrito => (
                  <article
                    key={inscrito.id}
                    className="rounded-xl border border-accent/70 bg-surface p-4 shadow-sm"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="truncate font-semibold">{inscrito.nome}</p>
                        <div className="mt-1 flex items-center gap-2">
                          <p className="text-sm text-primary/70">{inscrito.whatsapp}</p>
                          <button
                            type="button"
                            onClick={() => copiarTelefone(inscrito.whatsapp)}
                            title="Copiar telefone"
                            className="inline-flex h-7 w-7 items-center justify-center rounded-md border border-accent/70 bg-surface text-primary transition hover:bg-secondary/10"
                          >
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div>
                        ...
                        <p className="mt-1 font-medium">
                          {formatMoeda(getValorInscricao(inscrito.created_at))}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs font-medium text-primary/55">Data</p>
                        <p className="mt-1 font-medium">
                          {formatDataCadastro(inscrito.created_at)}
                        </p>
                      </div>
                      <div>
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      ...
                      <button
                        type="button"
                        aria-label={`Copiar telefone de ${inscrito.nome}`}
                        onClick={() => copiarTelefone(inscrito.whatsapp)}
                        className="flex h-11 w-11 items-center justify-center rounded-xl border border-accent/70 bg-surface text-primary transition hover:border-primary"
                      >
                  ...
                  <tbody>
                    {inscritosFiltrados.length === 0 ? (
                      <tr>
                        <td
                          colSpan={7}
                          className="whitespace-nowrap px-4 py-4 text-center text-primary/70"
                        >
                          Nenhuma inscrita encontrada.
                        </td>
                      </tr>
                      ...
                      inscritosFiltrados.map(inscrito => (
                        <tr key={inscrito.id} className="border-t border-primary/10">
                          ...
                          <td className="whitespace-nowrap px-4 py-3">
                            <div className="flex items-center gap-2">
                              <span className="truncate">{inscrito.whatsapp}</span>
                              <button
                                type="button"
                                onClick={() => copiarTelefone(inscrito.whatsapp)}
                                title="Copiar telefone"
                            ...
                            <div className="flex gap-2">
                              ...
                              {!isPagamentoPago(inscrito.status_pagamento) && (
                                <button
                                  ...
                                  disabled={actionLoadingId === inscrito.id}
                                  onClick={() =>
                                    abrirConfirmacaoPagamento(inscrito.id, inscrito.nome)
                                  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary/40 p-4">
          <div className="w-full max-w-md rounded-2xl border border-accent/70 bg-surface p-5 shadow-xl">
            <h2 className="text-lg font-semibold">
              Confirmar pagamento de {confirmacaoPagamento.nome}?
            </h2>
              ...
              <select
                ...
                onChange={event =>
                  setMetodoPagamentoSelecionado(
                    event.target.value as (typeof METODOS_PAGAMENTO)[number]['value']
                  )
                }
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              >
                <Check className="h-4 w-4" />
                {actionLoadingId === confirmacaoPagamento.id
                  ? 'Confirmando...'
                  : 'Confirmar pagamento'}
              </button>
            </div>
